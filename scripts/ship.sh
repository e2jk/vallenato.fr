#!/usr/bin/env bash
# scripts/ship.sh — land your local work on master.
#
# Rebases the current branch onto the latest origin/master — any commits from
...
# opens (or reuses) a PR to master and enables auto-merge: it lands on its
# own once CI is green, no waiting around and no separate manual sync needed
# before your next round of commits — just run this script again.
#
# This is for the repo owner's own solo commits only — external
# contributors should keep using the normal GitHub fork-and-PR flow straight
# into master, which this script has no part in and doesn't affect.
#
# Adapted from OpenHangar's scripts/ship.sh (same logic; that repo's default
# branch is `main`, this one's is `master`).
#
# Usage: scripts/ship.sh [--no-verify]
#   --no-verify   skip the pre-push hook (ruff/mypy/bandit/zizmor/
#                 actionlint/pip-audit/coverage) — same flag, same meaning
#                 as `git push --no-verify`, just passed straight through.
set -euo pipefail
...
# about that conclusion depends on rebasing onto origin/master first.
...
# exactly on origin/master means there's nothing of ours left to land
# either: every commit we had was already merged upstream (that's what the
# rebase above just dropped). Pushing anyway would force-push a `ship` ref
# identical to master, which auto-pr-merge.yml's PR creation then rejects
# with "No commits between master and ship" — a confusing failure for a
# no-op.
if [ "$(git rev-parse HEAD)" = "$(git rev-parse origin/master)" ]; then
  echo "Nothing to ship — HEAD is already even with origin/master. Not pushing."
  exit 0
fi
