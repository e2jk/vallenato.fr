    ...
    if f[-5:] == ".html" and f != "index.html":
        ...
        with open(os.path.join(folder, f)) as in_file:
            ...
            if not r:
                ...
                author = r.group(2)
            if not r:
                raise ValueError(f"Could not find a <title> tag in {f}")
            title = r.group(1)
            ...
            if full_tuto_pos > 0:
                # print(interesting_part)
                videos_full_tutorial_content = interesting_part[
                    full_tuto_pos + 25 : full_version_pos - 8
                ]
                videos_full_tutorial_content = re.sub(
            ...
            full_version = interesting_part[
                full_version_pos + 19 : full_version_pos + 30
            ]
            # print(full_version)
            # print()
            ...
            output.append(
                {
                    ...
                    "title": title,
                    "videos": videos,
                    "videos_full_tutorial": videos_full_tutorial,
...
js_content = f"var tutoriales = {output_content};"
print(js_content)
